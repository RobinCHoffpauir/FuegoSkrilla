...
original_author="RobinCHoffpauir"
original_name="fuegoskrilla"
original_urlname="FuegoSkrilla"
original_description="Awesome fuegoskrilla created by RobinCHoffpauir" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/fuegoskrilla "${name}"
...
then
    ...
    echo "then run 'fuegoskrilla' or 'python -m fuegoskrilla'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'fuegoskrilla' or 'python -m fuegoskrilla' or 'poetry run fuegoskrilla'"
fi
