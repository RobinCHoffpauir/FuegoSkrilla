...
- 🎯 Entry points to execute your program using `python -m <fuegoskrilla>` or `$ fuegoskrilla` with basic CLI argument parsing.
...
---
# fuegoskrilla Flask Application

Awesome fuegoskrilla created by RobinCHoffpauir
...
```bash
git clone https://github.com/RobinCHoffpauir/FuegoSkrilla fuegoskrilla
cd fuegoskrilla
make install
```
...
```bash
fuegoskrilla create-db   # run once
fuegoskrilla populate-db  # run once (optional)
fuegoskrilla add-user -u admin -p 1234  # ads a user
fuegoskrilla run
```
